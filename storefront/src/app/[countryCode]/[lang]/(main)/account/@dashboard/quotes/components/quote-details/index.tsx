...
type QuoteDetailsProps = {
  quote: StoreQuoteResponse["quote"] & { customer: B2BCustomer }
  preview: AdminOrderPreview
  countryCode: string
  lang: string
}

const QuoteDetails: React.FC<QuoteDetailsProps> = ({ quote, preview, countryCode, lang }) => {
  ...
  return (
    <div className="flex flex-col gap-y-2 p-0">
      <div className="flex gap-2 justify-between items-center mb-2">
        <LocalizedClientLink
          href={accountPath({ countryCode, lang }, "quotes")}
        ...
        <div className="small:col-span-4 flex flex-col gap-y-2">
            ...
            <Container className="p-0">
              <div className="flex items-center justify-between px-6 py-4">
                ...
                <Button size="small" onClick={() => {
                  router.push(accountPath({ countryCode, lang }, `orders/details/${quote.draft_order_id}`))
                }}>
                  View Order
                </Button>
              </div>
          ...
          <Container className="p-0">
            <div className="py-4">
              <div className="flex items-center justify-between mb-2 px-6">
                <span className="txt-small text-ui-fg-subtle font-semibold">Current Total</span>
                <span className="txt-small text-ui-fg-subtle">{formatAmount(order.total, order.currency_code)}</span>
              </div>
              <div className="flex items-center justify-between px-6">
                <span className="txt-small text-ui-fg-subtle font-semibold">New Total</span>
                <span className="txt-small text-ui-fg-subtle">{formatAmount(preview.total, order.currency_code)}</span>
              </div>
            </div>
          </Container>
          {quote.status === "pending_customer" && (
            <div className="flex gap-x-3 justify-end my-4">
              ...
              >
                <Button size="small" variant="secondary">Reject Quote</Button>
              </PromptModal>
              <PromptModal
                title="Accept Quote?"
              ...
              >
                <Button size="small" variant="primary">Accept Quote</Button>
              </PromptModal>
            </div>
          )}
        ...
        <div className="col-span-2 flex flex-col gap-y-2">
          <Container className="flex gap-x-3 justify-between">
            <div className="text-sm">
              <span className="font-semibold text-ui-fg-subtle">Quote ID:</span> #<span>{quote.draft_order.display_id}</span>
            </div>
          ...
          <Container>
            ...
            <div className="text-sm text-ui-fg-subtle">
              <div className="flex justify-between"><Text>Email</Text><Text>{quote.customer?.email || "-"}</Text></div>
              <div className="flex justify-between"><Text>Phone</Text><Text>{quote.customer?.phone || "-"}</Text></div>
              <div className="flex justify-between"><Text>Spend Limit</Text><Text>{(quote.customer?.employee?.spending_limit && formatAmount(quote.customer?.employee?.spending_limit || 0, order.currency_code.toUpperCase())) || "-"}</Text></div>
            </div>
          </Container>
          <Container>
            <Heading level="h3" className="mb-2">Company</Heading>
            <div className="text-sm text-ui-fg-subtle">
              <div className="flex justify-between"><Text>Name</Text><Text>{quote.customer?.employee?.company?.name || "-"}</Text></div>
            </div>
          </Container>
        </div>
      </div>
