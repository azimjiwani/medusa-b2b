...
const defaultValues = { text: "", item_id: undefined }

const QuoteMessages = ({ quote, preview }: { quote: StoreQuoteResponse["quote"]; preview: AdminOrderPreview }) => {
  ...
  const originalItemsMap = useMemo(() => new Map<string, AdminOrderLineItem>(
    quote.draft_order?.items?.map((item: AdminOrderLineItem) => [item.id, item])
  ), [quote.draft_order])
  const previewItemsMap = useMemo(() => new Map<string, AdminOrderLineItem>(
    ...
    <Container className="divide-y divide-dashed p-0 ">
      ...
      <div>
        {quote.messages?.map((message) => (
          <div
            key={message.id}
            className={clx("px-6 py-4 text-sm flex flex-col gap-y-2", { "!bg-ui-bg-subtle !inset-x-5 !inset-y-3": !!message.customer_id })}
          >
            <div className="font-medium font-sans txt-compact-small text-ui-fg-subtle ">
              {!!message.admin && `${message.admin.first_name} ${message.admin.last_name}`}
              {!!message.customer && `${message.customer.first_name} ${message.customer.last_name}`}
            </div>
            {!!message.item_id && (
      ...
      <div className="px-4 pt-5 pb-3">
        <form onSubmit={handleSubmit(handleCreateMessage)} className="flex flex-col gap-y-3">
          <div className="flex items-center gap-3">
            <div className="flex-1">
              <div className="flex items-center gap-x-1">
                <label className="font-sans txt-compact-small font-medium">Pick Quote Item</label>
              </div>
              <span className="txt-small text-ui-fg-subtle" id=":r10:-form-item-description">Select a quote item to write a message around</span>
            </div>
            <div className="flex-1">
              <Controller name="item_id" control={control} render={({ field: { onChange, ref, value, ...field } }) => (
                <Select {...field} onValueChange={onChange} value={value}>
                  ...
                  <Select.Content>
                    {preview?.items?.map((l) => (
                      <Select.Item key={l.id} value={l.id}>{l.variant_sku}</Select.Item>
                    ))}
                  </Select.Content>
                </Select>
              )} />
          ...
          <Button size="small" type="submit" className="self-end" disabled={isCreatingMessage}>Send</Button>
        </form>
      </div>
    </Container>
  )
