...
export const metadata: Metadata = { title: "Search Results", description: "Search our products." }

type Params = {
  searchParams: Promise<{ q?: string; sortBy?: SortOptions; category?: string; page?: string }>
...
export default async function SearchPage(props: Params) {
  ...
  return (
    <div className="bg-neutral-100">
      <div className="flex flex-col py-6 content-container gap-4" data-testid="search-container">
        <StoreBreadcrumb />
        {searchQuery && (<div className="mb-2"><h1 className="text-2xl font-semibold">Search Results for: {searchQuery}</h1></div>)}
        <SearchPageWrapper sortBy={sort} categories={categories} currentCategory={currentCategory} searchQuery={searchQuery || ""} hideSearch={true}>
          <Suspense fallback={<SkeletonSearchGrid />}> 
            <FilteredSearchResults searchQuery={searchQuery || ""} countryCode={params.countryCode} customer={minimalCustomerInfo} page={pageNumber} sortBy={sort} />
          </Suspense>
        </SearchPageWrapper>
      </div>
    </div>
